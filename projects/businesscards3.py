...
def generatemodulation(len,rangex, base, scale):
	a = []
	for x in range(len):
		# a.append(random.randint(0,range))
		# print(x)
		# v = scale * pnoise1(float(x + base) / len, 16)
		v = scale * random.random() * (float(x + base) / len)
		# if (v < 0):
		# 	v = v*0.01
		# a.append(v)
		a.append(v)
...
def renderline(indata, moddata, miny, maxy):
	...
	g = shapes.group([])
	# random.seed(10)
	for yi in range(len(moddata)):
		yoffset = spacingA * pow(spacingB, yi)
		points = []
		for xi in range(len(indata)):
			x = xi * (spacingC/(yi +1))
			if (x < pltmax[0]):
...
def subdivpattern(xoffset,yoffset,width,height,generations,numtypes):
    ...
    for gen in range(generations):
        ...
        for sq in zones[gen]:
            ...
            x = random.randint(int(sq[0]),int(sq[0]+(sq[2]-sq[0])/2))
            y = random.randint(int(sq[1]),int(sq[1]+(sq[3]-sq[1])/2))
...
def writeaddress(address, adresszone):
    ...
    for i,l in enumerate(address):
        ...
        t =  shapes.label(str(l), charwidth, charwidth)
